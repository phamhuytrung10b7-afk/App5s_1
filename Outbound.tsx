...
import { Upload, CheckCircle, XCircle, Search, Settings, ArrowRightLeft, Users, Warehouse, Store, AlertTriangle, ArrowRight, History, Plus, Calendar, ChevronDown, ChevronUp, ListFilter } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
         <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-slate-500 text-sm">Xuất bán, điều chuyển và tra cứu lịch sử.</p>
             </div>
         </div>
         <div className="flex bg-slate-100 p-1 rounded-lg">
             <button 
                onClick={() => setActiveTab('SCAN')}
                className={`px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'SCAN' ? 'bg-white shadow text-blue-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
                <Plus size={16}/> Xuất Mới
             </button>
             <button 
                onClick={() => setActiveTab('HISTORY')}
                className={`px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'HISTORY' ? 'bg-white shadow text-blue-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
         ...
         <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 space-y-6">
             <div className="flex flex-col md:flex-row gap-4 items-end border-b border-slate-100 pb-6">
                <div>
                   ...
                   <input 
                      type="date" 
                      className="p-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-400"
                      value={historyFrom}
                ...
                <div className="flex-1 text-right">
                   <p className="text-sm text-slate-500">Tìm thấy <span className="font-bold text-slate-800">{historyData.length}</span> giao dịch.</p>
                </div>
             </div>
             ...
             <div className="space-y-4">
                      ...
                      <div key={tx.id} className="border border-slate-200 rounded-lg overflow-hidden transition-all hover:border-blue-300">
                         ...
                         >
                            <div className="flex items-center gap-4">
                               <div className={`p-2 rounded-full ${isSale ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                                  {isSale ? <Store size={18} /> : <ArrowRightLeft size={18} />}
                               </div>
                               <div>
                                  <div className="font-bold text-slate-800">{new Date(tx.date).toLocaleString('vi-VN')}</div>
                                  <div className="text-sm text-slate-500">
                                     {product?.model || tx.productId} 
                                     <span className="mx-2">•</span> 
                                     {isSale ? `Khách: ${tx.customer}` : `Đến: ${tx.toLocation}`}
                                  </div>
                               </div>
                            </div>
                            <div className="flex items-center gap-4">
                               <div className="text-right">
                                  <div className="font-bold text-blue-600 text-lg">-{tx.quantity}</div>
                ...
                {historyData.length === 0 && (
                   <div className="text-center py-12 text-slate-400 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                      Không có giao dịch xuất kho trong khoảng thời gian này.
                   </div>
                )}
             </div>
         </div>
