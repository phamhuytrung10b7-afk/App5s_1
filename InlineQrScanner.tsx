...
export const InlineQrScanner: React.FC<InlineQrScannerProps> = ({
  ...
  return (
    <div className={`p-4 sm:p-5 rounded-2xl shadow-md space-y-4 border-2 transition-all ${
      mode === 'in' 
        ? 'bg-gradient-to-br from-emerald-50 via-teal-50 to-slate-50 border-emerald-400 text-slate-900'
        : 'bg-gradient-to-br from-indigo-50 via-sky-50 to-blue-50 border-indigo-400 text-slate-900'
    }`}>
      {/* Header Banner */}
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-[11px] text-slate-600 font-semibold">Sẵn sàng nhận tín hiệu súng quét USB/Camera</span>
            </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {(scanInput || lastScannedPart || errorMsg) && (
            <button
              type="button"
              onClick={handleReset}
              className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-800 rounded-xl text-xs font-bold transition-all flex items-center space-x-1 cursor-pointer"
            >
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input
            ...
            className="w-full pl-4 pr-24 py-3.5 bg-white border-2 border-indigo-500 focus:border-indigo-600 focus:ring-4 focus:ring-indigo-200 text-indigo-950 font-mono font-bold text-sm sm:text-base rounded-xl placeholder:text-slate-400 placeholder:font-normal placeholder:text-xs outline-hidden shadow-xs"
          />
          ...
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 text-white text-xs font-black rounded-lg transition-all cursor-pointer"
          >
            Nhận mã
          </button>
        </div>
        <p className="text-[11px] text-slate-600 flex items-center space-x-1 font-medium">
